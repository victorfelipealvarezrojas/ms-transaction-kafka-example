...
public class KafkaConfig {
	...
	public Map<String, Object> producerConfigs() {
		...
		// esto es necesario para la transacción y que sean confirmados todos los mensajes (o ninguno)
	...
	// estos mensajes van a ser enviados en la misma transacción gracias a TRANSACTIONAL_ID_CONFIG
	@Bean
	...
	/**
	 * Bean que gestiona las transacciones distribuidas en Kafka.
	 * Requerido para que las anotaciones @Transactional funcionen con operaciones Kafka.
	 * Asegura atomicidad: si falla cualquier operación, hace rollback de todos los mensajes
	 * enviados a los tópicos (ej.: withdraw-money y deposit-money).
	 */
	...
	// topico para retiro de dinero
	@Bean
	NewTopic createWithdrawTopic() {
		return TopicBuilder
				.name(withdrawTopicName)
				.partitions(3)
				.replicas(3)
				.build();
	}

	// topico para depósito de dinero
	@Bean
	NewTopic createDepositTopic() {
		return TopicBuilder
				.name(depositTopicName)
				.partitions(3)
				.replicas(3)
				.build();
	}
